...
export default function Expenses() {
	...
	const { data: expenses, refetch } = api.expense.getExpenses.useQuery(budgetId);
    ...
    //sort expenses into tag amounts
	if (expenses) {
		for (const expense of expenses) {
			for (const tag of expense.tags) {
				const existingTag = tagAmounts.find(
	...
	return (
		<main className="flex flex-col p-4 px-6">
			...
			<div className="mb-4 px-4 py-1 font-mono text-2xl text-muted-foreground">
				{budget?.name}
			</div>

			<div className="flex items-center justify-start space-x-4">
				<Card className="h-38 w-48">
					<CardHeader>
							...
							<div className="grid gap-4">
								<div className="grid gap-3">
									...
									<Input
										id="amount-1"
										name="amount"
										value={addBudgetAmount === 0 ? "" : addBudgetAmount}
					...
					<CardHeader>
						...
						<CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl">
							{expenses
								?.filter(
									(expense) =>
										(expense.createdAt.getMonth() === month.month &&
											expense.createdAt.getFullYear() === month.year) ||
										month.year === 0,
								)
                ...
                {
                    tagAmounts.length > 0 && tagAmounts.map((tag) => (
                        <Card className="w-48 h-38 inset-ring-2 inset-ring-white" key={tag.id}>
                            <CardHeader>
                                <CardDescription>{tag.name}</CardDescription>
                                <CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl">
                                    {tag.amount.toLocaleString("en-US", {
                                        style: "currency",
                                        currency: "USD",
                                    })}
                                </CardTitle>
				...
				<DialogContent className="sm:max-w-[425px]">
					...
					<DialogFooter>
						...
						<Button
							type="submit"
							onClick={() =>
								addContributor.mutate({ budgetId, email: contributorEmail })
							}
						>
							Save changes
						</Button>
			...
			<Button
				...
				onClick={() => removeBudget.mutate(budgetId)}
			>
				Delete Budget
			</Button>
		</main>
